...
function optionsFor(g, stage){
  ...
  if (el.t === "icon") {
    ...
    var ids = Object.keys(V).filter(function(k){
      var vb = V[k];
      if (vb.needs === "stroke" && !hasStroke(svg)) return false;
      if (vb.needs === "second-state" && !el.f.secondState) return false;
      if (vb.needs === "in-progress" && !el.f.loading) return false;
      if (vb.cont && !el.f.loading) return false;
      return true;
    });
...
function mainMotionOf(fr, stage){
  ...
  if (stage === "Confirm" || stage === "Welcome") {
    for (i=0;i<gs.length;i++) if (gs[i].el.t === "icon") return gs[i].key;
    for (i=0;i<gs.length;i++) if (kindOf(gs[i].el) === "single") return gs[i].key;
  }
  return null;
}
...
function promptFor(id, g, frameName, isMain){
  ...
  if (window.VERBS[id]) L.push("One unbroken movement, one property.");
...
function keyframesFor(id){
  ...
  return { track:track, durationSeconds:secs, stagger: p.st ? p.st/1000 : 0,
